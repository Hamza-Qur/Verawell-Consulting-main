...
const AttendanceDataTable = () => {
  ...
  const facilities = ['All', 'KFC Facility', 'Starbucks Facility', 'Burger King Facility'];
  ...
  const handleEdit = (rowData) => {
    alert(`Edit attendance for: ${rowData.employeeName}`);
    setDropdownOpen(null);
  };

  const handleDelete = (rowData) => {
    if (window.confirm(`Are you sure you want to delete attendance record for ${rowData.employeeName}?`)) {
      alert(`Deleted attendance record for ${rowData.employeeName} successfully`);
    }
    setDropdownOpen(null);
  };
  ...
  const attendanceData = [
    ...
    { 
      ...
      id: '22739',
      facility: 'Starbucks',
      checkinTime: '09:06:45',
      checkoutTime: '16:40:23',
      status: 'Leave',
      hoursWorked: '90'
    },
    { 
  ...
  const filteredData = selectedFacility === 'All' 
    ...
    : attendanceData.filter(employee => {
        if (selectedFacility === 'KFC Facility') return employee.facility === 'KFC';
        if (selectedFacility === 'Starbucks Facility') return employee.facility === 'Starbucks';
        if (selectedFacility === 'Burger King Facility') return employee.facility === 'Burger King';
        return true;
      });
  ...
  const columns = [
    ...
    },
    {
      ...
      options: {
        customBodyRender: (value) => {
          const color = value === '---' ? '#666' : '#D32F2F';
          const fontWeight = value === '---' ? '400' : '600';
          return (
          ...
          {facilities.map((facility) => (
            <button
              ...
              style={{
                ...
                borderColor: selectedFacility === facility ? '#8B2885' : 'transparent',
                backgroundColor: selectedFacility === facility ? 'transparent' : 'transparent',
