...
const handleDownload = (rowData) => {
  try {
    ...
    link.download = `${rowData.documentName.replace(/\s+/g, '_')}_${rowData.uploadedBy.replace(/\s+/g, '_')}.pdf`;
...
const handleAttachToEmail = async (rowData) => {
  try {
    ...
    const pdfFile = new File([pdfBlob], 
      `${rowData.documentName.replace(/\s+/g, '_')}_${rowData.uploadedBy.replace(/\s+/g, '_')}.pdf`, 
    ...
    reader.onloadend = () => {
      ...
      const subject = encodeURIComponent(`${rowData.documentName} - ${rowData.uploadedBy}`);
      const body = encodeURIComponent(
        ...
        `File: ${rowData.documentName.replace(/\s+/g, '_')}_${rowData.uploadedBy.replace(/\s+/g, '_')}.pdf`
      );
      ...
      setTimeout(() => {
        alert(`PDF has been prepared for email.\n\n` +
              `File: ${pdfFile.name}\n` +
  ...
  const handleDelete = (rowData) => {
    if (window.confirm(`Are you sure you want to delete ${rowData.documentName} for ${rowData.uploadedBy}?`)) {
    ...
    <>
      ...
      {dropdownOpen !== null && createPortal(
        ...
        >
          ...
          >
            <Icon icon="mdi:download-outline" width="18" height="18" color="#666" /> 
          ...
          >
            <Icon icon="mdi:email-outline" width="18" height="18" color="#666" /> 
